...
const char * myWriteAPIKey = "6NVF9Z1DQ2KV6UDB";
...
void thingspeakpush() {
  ...
  if(isnan(humid)| humid > 100){ // if dht11 failed
    status = String("DHT11 Failed, restarting");
    ThingSpeak.setStatus(status);
    ESP.restart();
  }
  else if(isnan(temp) | isnan(baro) | bmpfail == true){ // if bmp280 failed
    status = String("BMP280 Failed, restarting");
    ThingSpeak.setStatus(status);
    ESP.restart();
  }
